...
/-- One pending migration as `status` reports it: its steps with row
    ...
    Also the tables whose physical definition this migration replaces or
    removes (`rebuildTable` / `dropTable`): the import report keys the
    uncarried-index/view warning on them. -/
private def pendingJson (conn : Conn) (prev : List TableSpec) (m : Migration) (version : Nat) :
    IO (Except String (Json × Bool × Array (String × String))) := do
  ...
  | .ok plan =>
      ...
      -- which tables this migration rebuilds or drops, and the verb for
      -- the warning: a rebuild or drop physically removes the table, so
      -- its uncarried indexes die with it and views referencing it break
      let covered : Array (String × String) := Id.run do
        let mut out : Array (String × String) := #[]
        for st in plan.steps do
          match st with
          | .rebuildTable spec _ => out := out.push (spec.name, "rebuilds")
          | .dropTable t => out := out.push (t, "drops")
          | _ => pure ()
        return out
      return .ok (Json.mkObj [
        ...
        ("steps", Json.arr steps)], destructive, covered)
...
/-- The importer's `import-report.json`, when the base was generated by
    `leandb import-sqlite` and the report sits next to the instance: the
    instance's directory, or the package root above it (the generated
    layout). Absent file, absent parsing → `none`, and `migrate status`
    stays silent. -/
def importReport? (inst : Instance) : IO (Option Json) := do
  let dir := inst.path.parent.getD "."
  let candidates := [dir / "import-report.json", dir / ".." / "import-report.json"]
...
/-- `migrate status`'s rebuild warning: when a pending migration rebuilds
    or drops a table that the import report lists as owning uncarried
    indexes or views, the rebuild drops those indexes with the table and
    invalidates those views. Pure over the parsed report, so tests drive
    it directly. -/
def rebuildReportNotes (report : Json) (covered : Array (Nat × String × String)) :
    Array String := Id.run do
  let entries := match report.getObjValAs? (Array Json) "notCarried" with
    | .ok es => es | .error _ => #[]
  let owned := fun (kind t : String) => Id.run do
    let mut out : Array String := #[]
    for e in entries do
      if (e.getObjValAs? String "kind").toOption == some kind then
        let tables := (e.getObjValAs? (Array Json) "tables").toOption.getD #[]
        if tables.any fun j => (j.getStr?).toOption == some t then
          if let some n := (e.getObjValAs? String "name").toOption then
            out := out.push n
    return out
  let mut out : Array String := #[]
  for (v, t, verb) in covered do
    let idxs := owned "index" t
    let views := owned "view" t
    if idxs.isEmpty && views.isEmpty then continue
    let mut parts : Array String := #[]
    unless idxs.isEmpty do
      parts := parts.push s!"will drop uncarried index(es) {String.intercalate ", " idxs.toList}"
    unless views.isEmpty do
      parts := parts.push s!"will invalidate view(s) {String.intercalate ", " views.toList}"
    out := out.push
      s!"pending V{v} {verb} \"{t}\": {String.intercalate "; " parts.toList} (see import-report.json)"
  return out

/-- The status warning for a pending rebuild: reads the report next to the
    instance; a missing report stays silent. -/
def importReportNotes (inst : Instance) (covered : Array (Nat × String × String)) :
    IO (Array String) := do
  let r? ← importReport? inst
  return (r?.map fun r => rebuildReportNotes r covered).getD #[]
...
private def chainMigrate (b : Base) (inst : Instance) (sess : Session) (c : Chain)
  ...
  if !apply then
    ...
    let mut covered : Array (Nat × String × String) := #[]
    ...
    for m in pending do
      ...
      | .ok (j, d, cov) =>
          ...
          covered := covered.append (cov.map fun (t, verb) => (v, t, verb))
      ...
      prev := m.snapshot
    -- the import report, when the base was generated by `leandb
    -- import-sqlite` and the report sits next to the instance: warn by
    -- name that the pending rebuild drops its uncarried indexes and
    -- invalidates its views; a missing report stays silent
    notes := notes.append (← importReportNotes inst covered)
