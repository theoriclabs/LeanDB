...
instance (priority := low) {α : Type} [Entity α] : HasUnique α where
  Unique := Empty
  Key := fun x => nomatch x
...
instance (priority := low) {α : Type} [Entity α] : HasForeignKey α where
  ForeignKey := Empty
...
instance (priority := low) {α : Type} [Entity α] : HasListField α where
  ListField := Empty
...
instance (priority := low) {s α : Type} : HasReferencedBy s α where
  ReferencedBy := Empty
