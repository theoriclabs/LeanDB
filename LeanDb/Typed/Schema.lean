...
class HasReferencedBy (s : Type) (α : Type) where
  ReferencedBy : Type
...
@[reducible] instance (priority := low) {s α : Type} : HasReferencedBy s α where
  ReferencedBy := Empty
...
abbrev ReferencedBy.Source {s α : Type} [HasReferencedBy s α] (r : ReferencedBy s α) : Type :=
  HasReferencedBy.Source r

def ReferencedBy.sourceEntity {s α : Type} [HasReferencedBy s α] (r : ReferencedBy s α) :
    Entity (ReferencedBy.Source r) :=
  HasReferencedBy.sourceEntity r

def ReferencedBy.getFk {s α : Type} [HasReferencedBy s α] (r : ReferencedBy s α)
    (v : ReferencedBy.Source r) : Id α :=
  HasReferencedBy.getFk r v

def ReferencedBy.columnName {s α : Type} [HasReferencedBy s α] (r : ReferencedBy s α) : String :=
  HasReferencedBy.columnName r

def ReferencedBy.sourceName {s α : Type} [HasReferencedBy s α] (r : ReferencedBy s α) : String :=
...
def elabSchema : CommandElab := fun stx => do
  ...
  -- Marker type and table enum.
  elabCommand (← `(structure $(rootIdent schemaName):ident where deriving Repr))
  let tableName := schemaName ++ `Table
  let tableCtors : TSyntaxArray `Lean.Parser.Command.ctor ← types.mapM fun t =>
    `(Lean.Parser.Command.ctor| | $(mkIdent (ctorOfType t)):ident)
  elabCommand (← `(
    inductive $(rootIdent tableName):ident where $tableCtors* deriving DecidableEq, Repr
  ))
  ...
  elabCommand (← `(
    instance : LeanDb.IsSchema $schemaId where
      Table := $tableId
      tables := #[$tableVals,*]
  ...
  for t in types do
    let c := mkIdent (ctorOfType t)
    elabCommand (← `(
      instance : LeanDb.IsSchema.Has $schemaId $(typeIdent t) where
        id := .$c:ident
        ty_eq := rfl
    ))
